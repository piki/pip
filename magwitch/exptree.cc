	...
	if (node->type() == NODE_STRING)
		...
		switch (onode->op) {
			case '=':
				...
				return create(dynamic_cast<StringNode*>(onode->operands[1]), negate);
				break;
			case '!':
				assert(onode->nops() == 1);
				return create(dynamic_cast<StringNode*>(onode->operands[0]), !negate);
				break;
			default:
...
ExpTask::ExpTask(const OperatorNode *onode) {
	...
	name = Match::create(dynamic_cast<StringNode*>(task_decl->operands[0]), false);
...
ExpNotice::ExpNotice(const OperatorNode *onode) {
	...
	name = Match::create(dynamic_cast<StringNode*>(onode->operands[0]), false);
}
...
MatchSet ExpRepeat::check(const PathEventList &test, unsigned int ofs, const FutureTable &ft, const FutureCounts &fc) const {
	...
	for (count=0; count<max; count++) {
		if (count >= min) ret.insert(prev.begin(), prev.end());
		MatchSet newset;
	...
	if (count == max) // reached the end
		ret.insert(prev.begin(), prev.end());
	if (count < min && debug_failed_matches_individually)
...
PathGenerator::PathGenerator(const IdentifierNode *ident, const ListNode *statements) {
	assert(statements->type() == NODE_LIST);
	...
	assert(root);
	assert(root->max == 1);  // first thread is the root, can only appear once
}
...
void PathGenerator::print(FILE *fp) const {
