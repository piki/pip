...
static void gtk_graph_layout(GtkGraph *graph) {
	...
	switch ((pid = fork())) {
		...
		case 0:    /* child */
			...
			close(rfd[0]);
			close(rfd[1]);
			if (execlp(cmd, cmd, "-Tplain", NULL) == -1) {
				perror(cmd);
				exit(1);
			}
		default:
   ...
   * worse. */
	...
	while ((fgets(buf, sizeof(buf), rfp)) != NULL) {
		...
		if (!strcmp(type, "node")) {
			...
			node->x = DOT_SCALE*graph->zoom*atof(strtok(NULL, " \t"));
			node->y = -DOT_SCALE*graph->zoom*atof(strtok(NULL, " \t"));
		}
			...
			if (npoints > 0) {
				...
				for (i=0; i<npoints; i++) {
					GdkPoint p;
					p.x = DOT_SCALE*graph->zoom*atof(strtok(NULL, " \t"));
					p.y = -DOT_SCALE*graph->zoom*atof(strtok(NULL, " \t"));
